...
function grounded(context,kind){
  ...
  return patterns[kind]?patterns[kind].test(c):false;
}
function collectReplyQualityIssues(replies,context){
  const issues=[]; const texts=(Array.isArray(replies)?replies:[]).map(x=>String(x?.text||"").trim()).filter(Boolean);
  if(texts.length<3) issues.push("추천 답장이 3개가 아님");
  for(let i=0;i<texts.length;i++){
    const t=texts[i], n=String(i+1);
    if(t.length>140) issues.push("답장 "+n+"이 너무 김");
    if(/(?:상대방|사용자는|AI|분석 결과|추천 답장)/.test(t)) issues.push("답장 "+n+"에 메타 표현 포함");
    if(/아직\s*안\s*봤는데|그냥\s*평범한\s*것\s*같은데/.test(t)) issues.push("답장 "+n+"이 질문 맥락과 어색함");
    if(/(?:바빠|바쁠|시간\s*없|출근|퇴근|회사\s*가|근무)/.test(t) && !grounded(context,"busy")) issues.push("답장 "+n+"이 사용자 바쁨/근무를 지어냄");
    if(/(?:집에\s*있|집에서\s*있)/.test(t) && !grounded(context,"home")) issues.push("답장 "+n+"이 사용자 위치를 지어냄");
    if(/(?:쉬고\s*있|쉴\s*거|쉬려고|쉬는\s*중)/.test(t) && !grounded(context,"rest")) issues.push("답장 "+n+"이 사용자 휴식 일정을 지어냄");
    if(/(?:약속|일정|계획)[^.!?]{0,10}(?:없|없어|없는데)/.test(t) && !grounded(context,"noPlan")) issues.push("답장 "+n+"이 약속 없음 사실을 지어냄");
    if(/(?:나도|저도)[^.!?]{0,20}(?:좋아|자주|즐겨)/.test(t) && !grounded(context,"commonLike")) issues.push("답장 "+n+"이 공통 취향을 지어냄");
    if(/(?:가봤|해봤|먹어봤)/.test(t) && !grounded(context,"experience")) issues.push("답장 "+n+"이 사용자 경험을 지어냄");
    if(/(?:생각났|기대돼|기대되|설레|보고\s*싶었)/.test(t) && !grounded(context,"emotion")) issues.push("답장 "+n+"이 사용자 감정을 지어냄");
    if(/(?:그때|전에|아까)[^.!?]{0,24}(?:말한|얘기한|이야기한)/.test(t) && !/(?:그때|전에|아까)[^.\n]{0,50}(?:말|얘기|이야기)/.test(String(context||""))) issues.push("답장 "+n+"이 없는 과거 대화를 만듦");
  }
  for(let i=0;i<texts.length;i++) for(let j=i+1;j<texts.length;j++) if(replySimilarity(texts[i],texts[j])>=0.72) issues.push("답장 "+String(i+1)+"과 "+String(j+1)+"이 너무 비슷함");
...
async function repairReplySet(replies,context,issues){
  ...
  const prompt=JSON.stringify({task:"아래 원문 맥락만 근거로 추천 답장 3개를 다시 작성하세요. 검수 문제를 모두 제거하세요.",context:String(context||"").slice(0,9000),detectedIssues:issues,currentReplies:(Array.isArray(replies)?replies:[]).slice(0,3),output:{replies:[{label:"자연스럽게",text:"짧은 답장",reason:"짧은 이유"},{label:"다정하게",text:"짧은 답장",reason:"짧은 이유"},{label:"여유 있게",text:"짧은 답장",reason:"짧은 이유"}]}});
  ...
  return parsed.replies.slice(0,3).map((x,i)=>({label:String(x?.label||["자연스럽게","다정하게","여유 있게"][i]),text:String(x?.text||"").trim(),reason:String(x?.reason||"").trim()}));
}
async function applyReplyQualityGate(payload,reqBody,pathName){
  ...
  const context=[reqBody?.message,reqBody?.text,reqBody?.conversation,reqBody?.selectedSituation,reqBody?.relation,reqBody?.starterGoal,reqBody?.goal,reqBody?.recentContext].filter(Boolean).map(String).join("\n");
  ...
  try{
    ...
    if(remaining.length){ repaired=await repairReplySet(repaired,context,remaining); remaining=collectReplyQualityIssues(repaired,context); }
    if(!remaining.length) return {...payload,replies:repaired,qualityGate:"repaired",qualityIssues:issues,serverVersion:SERVER_VERSION};
    console.warn("답장 품질 필터 재검수 미통과:",remaining,"path=",pathName);
  }catch(error){ console.error("답장 품질 재생성 실패:",error?.message||error,"path=",pathName); }
