...
const SERVER_VERSION = "2026-08-23-potentia-v38-critical-followups";
...
function getStarterGuard({message="",starterGoal="",selectedSituation="",relation=""}){
  ...
  if(/(?:자해\s*협박|자살|죽겠|죽어\s*버리|죽어버리|죽을\s*거|죽는다|극단적\s*선택)/.test(t)) return stop("자해 위협이 포함된 고위험 상황이에요.","연애 기술로 달래거나 관계를 유지하는 조건으로 책임을 떠안지 마세요. 급박한 위험이면 주변의 신뢰할 수 있는 사람이나 지역 응급·전문 도움을 연결하고, 사용자의 안전과 경계를 함께 지키는 것이 우선입니다.");
