...
const SERVER_VERSION = "2026-08-11-sse-v11";
...
function parseReplyObject(raw, fallbackLabel) {
  ...
  let recoveredText = textMatch ? textMatch[1] : text;
  ...
  if (recoveredText.startsWith("{")) recoveredText = "추천 문장을 생성했지만 마지막 부분이 잘렸어요. 다시 시도해주세요.";

  return { label: fallbackLabel || "추천", text: recoveredText, reason: "" };
}
...
async function streamClaudeSections({ res, model, maxTokens, content, sectionOrder }) {
  ...
  function tryEmit() {
    for (let i = 0; i < sectionOrder.length; i++) {
      ...
      if (end < 0 && streamFinished && i === sectionOrder.length - 1) {
        end = fullText.length;
      }
