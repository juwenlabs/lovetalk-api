...
const SERVER_VERSION = "2026-08-24-potentia-v78-openai-schedule-guard";
...
async function createFastStarterJson({content,advanced=false}){
...
async function generateStarterResult(reqBody){
  ...
  // v70: if the counterpart explicitly stated a preference and the goal is
...
function getInstantReplyCoreResult(reqBody){
  const mode=String(reqBody?.mode||"quick");
...
async function generateCompactReplyAnalysis(reqBody){
  const mode=String(reqBody?.mode||"quick");
